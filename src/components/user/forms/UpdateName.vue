...
const props = defineProps({
    ...
    title: { type: String, required: false, default: "user.ui.updateName" },
    // Prompt text. Hides the column if empty
    prompt: { type: String, required: false, default: "user.prompt.updateName" },
...
async function onSubmit(state) {
    await state.action(async (state) => {
        ...
        return props.service.updateName(state.values.name);
    });
}
</script>

...
export class DatabaseService extends Service {
     ...
     *
     * @async
     * @param {DocumentHandle} documentHandle - The document to delete
     * @returns {Promise} Resolves when successful
     */
    async deleteDoc(documentHandle) {
        if (documentHandle instanceof DocumentHandle) {
            return this._deleteDoc(documentHandle.handle);
        }

        throw new Error("Cannot delete document. No document given.");
    }
     ...
     *
     * @async
     * @param {any} args - The args passed to @see DatabaseService.deleteDoc - The doc is your native handle
     * @throws {Error} - If unimplemented
     * @returns {Promise<string>} Resolves/Fails.
     */
    async _deleteDoc(...args) {
        throw new Error("Abstract function called.");
    }
}
...
export class StorageService extends Service {
    ...
    static defaultConfig() {
        return {
            ...{
                // If defined, this is prepended to each path as the root inside the storage system
                root: "",
            },
            ...Service.defaultConfig(),
        };
    ...
    /**
     * The storage root prefix. If defined, this is appended to each storage path.
     *
     * @returns {String} storage prefix. If the given root is not a string, this returns "". If it is a string and ends
     * with "/", it stripped.
     */
    get root() {
        const root = this.config.root;
        if (root == null || !(typeof root === "string" || root instanceof String)) {
            return "";
        }
     ...
     *
     * @async
     * @param {StoragePathHandle|String} path - A path to fetch
     * @returns {Promise<Blob>} The data fetched as blob
     */
    async fetch(path) {
        if (path instanceof StoragePathHandle) {
            return this._fetch(path.handle);
        }

        if (Test.isNonEmptyString) {
            return this._fetch(this.path(path).handle);
        }

        throw new Error("Cannot retireve file. No valid path given.");
    }
     ...
     *
     * @async
     * @param {StoragePathHandle|String} path - A path to upload into
     * @param {File|Blob} fileBlob - A file or blob to upload
     * @returns {Promise} Resolve on success
     */
    async upload(path, fileBlob) {
        if (path instanceof StoragePathHandle) {
            return this._upload(path.handle, fileBlob);
        }

        if (Test.isNonEmptyString) {
            return this._upload(this.path(path).handle, fileBlob);
        }

        throw new Error("Cannot upload file. No valid path given.");
    }
     ...
     *
     * @async
     * @param {StoragePathHandle|String} path - The path to delete
     * @returns {Promise} Resolves on success
     */
    async delete(path) {
        if (path instanceof StoragePathHandle) {
            return this._delete(path.handle);
        }

        if (Test.isNonEmptyString) {
            return this._delete(this.path(path).handle);
        }

        throw new Error("Cannot delete file. No valid path given.");
    }

    /**
     * Fetch the given file as JSON
     *
     * @async
     * @param {StoragePathHandle|String} path - A path to fetch
     * @returns {Promise<Object>} The JSON object represented by the file.
     */
    ...
    /**
     * Create the URL that points to the file to load. Use this to generate the URL to put into img src, fetch (not
     * this.fetch), ...
     *
     * @async
     ...
     *
     * @param {Object} path - The path to fetch - as generated by _makePath
     *
     ...
     *
     * @param {Object} path - The path to load into - as generated by _makePath
     * @param {File|Blob} fileBlob - The data as file or blob
     *
     * @returns {Promise} Resolve on success
     */
    async _upload(path) {
     ...
     *
     * @async
     * @param {StoragePathHandle|String} path - The path to delete - as generated by _makePath
     * @returns {Promise} Resolves on success
     */
    async _delete(path) {
        throw new Error("Abstract function called.");
